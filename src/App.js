...
function App() {
  ...
  // onchangeの処理
  const textChange = (event, item, list, setList) => {
    const newText = event.target.value;
    // 更新関数を使って、同じidを持つ場合に...todoとnewTextを返す。
    setList(list.map(el => el.id === item.id ? { ...el, text: newText } : el));
  };

  // アイテムの移動処理
  /**
   * アイテムを移動させる関数。
   * @param {Object} item - 移動させたいアイテム。
   * @param {Array} sourceList - 現在存在するリスト。
   * @param {Function} setSourceList - リストを更新するための関数。
   * @param {Array} targetList - 移動先のリスト。
   * @param {Function} setTargetList - 移動先のリストを更新するための関数。
   * @param {string} text - 移動先のリストに追加するテキスト。
   */
  const moveItem = (item, sourceList, setSourceList, targetList, setTargetList, text) => {
    ...
    const updatedSourceList = sourceList.filter((el) => el.id !== item.id);
    // 移動元のリストを更新
        ...
        {todoList.map((item) => (
          <div key={item.id}>
            <input
              ...
              // onChangeを追加、テキスト取得。
              onChange={(event) => textChange(event, item, todoList, setTodoList)}
            />
      ...
      <div className="items">
        ...
        {inProgressList.map((item) => (
          <div key={item.id}>
            <input
              className="textbox" type="text" placeholder="taskを入力" defaultValue={item.text}
              onChange={(event) => textChange(event, item, inProgressList, setInProgressList)}
            />
      ...
      <div className="items">
        ...
        {doneList.map((item) => (
          <div key={item.id}>
            <input
              className="textbox" type="text" placeholder="taskを入力" defaultValue={item.text}
              onChange={(event) => textChange(event, item, doneList, setDoneList)}
            />
